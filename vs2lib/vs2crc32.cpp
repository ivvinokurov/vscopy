...
const unsigned int vs2crc32::crctab[256] = {0x00000000, 0x77073096, 0xee0e612c, 0x990951ba, 0x076dc419, 0x706af48f, 0xe963a535, 0x9e6495a3, 0x0edb8832, 
...
unsigned long int vs2crc32::calculate_crc(unsigned char *pBuf, unsigned long long length)
{

	unsigned long int c = 0xffffffff; // begin at shift register contents
	
	for (unsigned long long i = 0; i < length; i++)
	{
		c = crctab[(static_cast<int>(c) ^ pBuf[i]) & 0xff] ^ (c >> 8);
	}
	return c ^ 0xffffffff;
}

unsigned long int vs2crc32::begin_crc()
{
	return 0xffffffff;
}

unsigned long int vs2crc32::end_crc(unsigned long int crc)
{
	return crc ^ 0xffffffff;
}

unsigned long int vs2crc32::add_crc(unsigned long int oldcrc, unsigned char* pBuf, unsigned long long length)
{
	unsigned long int crc = oldcrc;
	...
	if (length > 0)
	{
		...
		for (i = 0; i < length; i++)
		{
			crc = crctab[(static_cast<int>(crc) ^ pBuf[i]) & 0xff] ^ (crc >> 8);
		}
	}
	return crc;
}
...
unsigned long int vs2crc32::add_crc(unsigned long int oldcrc, string value)
{
	unsigned char * pBuf = (unsigned char *)malloc(value.size());

	vs2comlib::convert_string_to_char(value, pBuf);

	unsigned long int v = vs2crc32::add_crc(oldcrc, pBuf, value.size());
...
unsigned long int vs2crc32::add_crc(unsigned long int oldcrc, long long value)
{
	unsigned char pBuf[8];

	vs2comlib::convert_long_to_char(value, pBuf);

	return vs2crc32::add_crc(oldcrc, pBuf, 8);
}

/// Calculate crc portion for int
unsigned long int vs2crc32::add_crc(unsigned long int oldcrc, long int value)
{
	unsigned char pBuf[4];

	vs2comlib::convert_int_to_char(value, pBuf);

	return vs2crc32::add_crc(oldcrc, pBuf, 4);
}

/// Calculate crc portion for short
unsigned long int vs2crc32::add_crc(unsigned long int oldcrc, short int value)
{
	unsigned char pBuf[2];
	
	vs2comlib::convert_short_to_char(value, pBuf);
	
	return vs2crc32::add_crc(oldcrc, pBuf, 2);
}

/// Calculate crc portion for char
unsigned long int vs2crc32::add_crc(unsigned long int oldcrc, unsigned char *value)
{
	return vs2crc32::add_crc(oldcrc, value, 1);
}
